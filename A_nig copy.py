...
def train_NIG(dataframe_call, dataframe_put, r, S0, T):
    def f_NIG(x, sigma, theta, kappa, payoff="call"):
    ...
    def obj_fun(params):
        try:
            # Call
            model = f_NIG(dataframe_call, params[0], params[1], params[2])
            ...
            # Put
            model = f_NIG(dataframe_put, params[0], params[1], params[2])
...
def get_NIG_pricer(params, strikes, r, S0, T, payoff="call"):
...
def calibrate_model(dataframe_call, dataframe_put, T, model="NIG", disp=False):
    MSE_list = []
    MSE_deep_itm_list = []
    MSE_mid_itm_list = []
    MSE_near_itm_list = []
    ...
    for exdate in dataframe_call.exdate.unique()[:40]:
        ...
        # Call
        params_NIG = train_NIG(call_exdate, put_exdate, r, S0, T)
        NIG_call = get_NIG_pricer(params_NIG, call_exdate.Strike.values, r, S0, T, payoff="call")
        prices_model_call = NIG_call.FFT(call_exdate.Strike.values)
        ...
        # Put
        NIG_put = get_NIG_pricer(params_NIG, put_exdate.Strike.values, r, S0, T, payoff="put")
        prices_model_put = NIG_put.FFT(put_exdate.Strike.values)
        ...
        for lower, upper, name in ranges:
            ...
            mse_call = np.mean((np.array(IV_model_call)[mask_call] - np.array(call_exdate.IV.values)[mask_call])**2)
            mse_put = np.mean((np.array(IV_model_put)[mask_put] - np.array(put_exdate.IV.values)[mask_put])**2)
            locals()[f'MSE_{name}_list'].append(mse_call + mse_put)
        ...
        if disp:
            ...
            plt.plot(call_exdate.Strike.values, IV_model_call, label='NIG IV')
            plt.scatter(put_exdate.Strike.values, put_exdate.IV.values, label='Actual IV', s=20, alpha=0.3)
            plt.plot(put_exdate.Strike.values, IV_model_put, label='NIG IV')
...
MSE_NIG_1, params_NIG_1 = calibrate_model(CALL_1dte, PUT_1dte, T=1/252, model="NIG", disp=True)
for column in MSE_NIG_1.keys():
    rmse_values = [np.sqrt(mse) for mse in MSE_NIG_1[column]]
    print(f"{column}: {np.nanmean(rmse_values):.3f} ({np.nanstd(rmse_values):.3f})")
pd.DataFrame(pd.DataFrame(MSE_NIG_1)).to_csv("data/results/MSE_NIG_1_2023_copy.csv", index=False)
raise Exception("Stop here")

MSE_NIG_7, params_NIG_7 = calibrate_model(CALL_7dte, PUT_7dte, T=5/252, model="NIG")
for column in MSE_NIG_7.keys():
    rmse_values = [np.sqrt(mse) for mse in MSE_NIG_7[column]]
    print(f"{column}: {np.nanmean(rmse_values):.3f} ({np.nanstd(rmse_values):.3f})")
pd.DataFrame(pd.DataFrame(MSE_NIG_7)).to_csv("data/results/MSE_NIG_7_2023_copy.csv", index=False)

MSE_NIG_30, params_NIG_30 = calibrate_model(CALL_30dte, PUT_30dte, T=22/252, model="NIG")
for column in MSE_NIG_30.keys():
    rmse_values = [np.sqrt(mse) for mse in MSE_NIG_30[column]]
    print(f"{column}: {np.nanmean(rmse_values):.3f} ({np.nanstd(rmse_values):.3f})")
pd.DataFrame(pd.DataFrame(MSE_NIG_30)).to_csv("data/results/MSE_NIG_30_2023_copy.csv", index=False)
