...
def train_Edgeworth(strikes, prices, spreads, r, S0, T, market_ivs):
    ...
    bounds = ([0.05, 0.01, -0.9, 0.001, 0.001, 0.001, 0.001, 0.001, 0.001], 
              [1.0, 5.0, 0.9, 0.1, 0.1, 0.1, 0.05, 0.05, 0.05])
    # bounds = [(0.05, 1.0), (0.01, 5.0), (-0.9, 0.9), (0.001, 0.1), (0.001, 0.1), (0.001, 0.1), (0.001, 0.05), (0.001, 0.05), (0.001, 0.05)]
    
    # params_Edgeworth = differential_evolution(obj_fun, bounds)
    params_Edgeworth = scpo.least_squares(obj_fun, x0=init_vals, bounds=bounds, method='trf')
...
MSE_Edgeworth_1, params_Edgeworth_1 = calibrate_model(CALL_1dte, T=1/252, model="Edgeworth")
pd.DataFrame(pd.DataFrame(MSE_Edgeworth_1)).to_csv("data/MSE_Edgeworth_1_2023.csv", index=False)

# MSE_Edgeworth_7, params_Edgeworth_7 = calibrate_model(CALL_7dte, T=5/252, model="Edgeworth")
# pd.DataFrame(pd.DataFrame(MSE_Edgeworth_7)).to_csv("data/MSE_Edgeworth_7_de.csv", index=False)

# MSE_Edgeworth_30, params_Edgeworth_30 = calibrate_model(CALL_30dte, T=22/252, model="Edgeworth")
# pd.DataFrame(pd.DataFrame(MSE_Edgeworth_30)).to_csv("data/MSE_Edgeworth_30_de.csv", index=False)
...
def greeks_function(strikes, prices, spreads, S0, IV_actual):
    params_Edgeworth = train_Edgeworth(strikes, prices, spreads, r, S0, T, IV_actual)
    Edgeworth = get_Edgeworth_pricer(params_Edgeworth, strikes, r, S0, T)
    deltas = Edgeworth.delta(strikes)
    gammas = Edgeworth.gamma(strikes)
