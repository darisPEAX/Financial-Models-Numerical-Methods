...
#     MSE_VG_1_ticker, params_VG_1_ticker = calibrate_model(CALL_1dte_2023_ticker, T=1/252, model="VG")
#     pd.DataFrame(pd.DataFrame(MSE_VG_1_ticker)).to_csv(f"data/results/MSE_VG_1_{ticker}.csv", index=False)
#     rmse_values = [np.sqrt(mse) for mse in MSE_VG_1_ticker["MSE_list"]]
...
def get_model_predicted(strikes, prices, spreads, r, S0, T, IV_actual):
    params = train_VG(strikes, prices, spreads, r, S0, T, IV_actual)
...
# greeks_df.to_csv("data/hedging_results/greeks/greeks_df_variancegamma.csv", index=False)
...
#     }
#     )).to_csv("data/hedging_results/delta/delta_pnl_list_2023.csv", index=False)
...
for ticker in ['aapl', 'amzn', 'msft']:
    ...
    print("Mean PnL: ", np.mean(delta_gamma_theta_pnl_list))
    print("Std PnL: ", np.std(delta_gamma_theta_pnl_list))
    greeks_df.to_csv(f"data/hedging_results/greeks/greeks_df_variancegamma_2023_{ticker}.csv", index=False)
    pd.DataFrame(pd.DataFrame(
        {
            "delta_pnl_list": delta_pnl_list,
            "delta_gamma_pnl_list": delta_gamma_pnl_list,
        ...
        )).to_csv(f"data/hedging_results/delta/delta_pnl_list_2023_variancegamma_{ticker}.csv", index=False)