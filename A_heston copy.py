...
def train_Heston(dataframe_call, dataframe_put, r, S0, T):
    def f_Heston(x, rho, sigma, theta, kappa, payoff="call"):
    ...
    def obj_fun(params):
        try:
            # Call
            model = f_Heston(dataframe_call, params[0], params[1], params[2], params[3], payoff="call")
            ...
            # Put
            model = f_Heston(dataframe_put, params[0], params[1], params[2], params[3], payoff="put")
...
def get_Heston_pricer(params, strikes, r, S0, T, payoff="call"):
...
def calibrate_model(dataframe_call, dataframe_put, T, model="Heston", disp=False):
    MSE_list = []
    MSE_deep_itm_list = []
    MSE_mid_itm_list = []
    MSE_near_itm_list = []
    ...
    for exdate in dataframe_call.exdate.unique()[:40]:
        ...
        params_Heston = train_Heston(call_exdate, put_exdate, r, S0, T)
        Heston_call = get_Heston_pricer(params_Heston, call_exdate.Strike.values, r, S0, T, payoff="call")
        Heston_put = get_Heston_pricer(params_Heston, put_exdate.Strike.values, r, S0, T, payoff="put")
        prices_model_call = Heston_call.FFT(call_exdate.Strike.values)
        prices_model_put = Heston_put.FFT(put_exdate.Strike.values)
        ...
        if disp:
            ...
            plt.plot(call_exdate.Strike.values, IV_model_call, label='Heston IV')
            plt.scatter(put_exdate.Strike.values, put_exdate.IV.values, label='Actual IV', s=20, alpha=0.3)
            plt.plot(put_exdate.Strike.values, IV_model_put, label='Heston IV')
...
# MSE_Heston_1, params_Heston_1 = calibrate_model(CALL_1dte_2023, PUT_1dte_2023, T=1/252, model="Heston")
# for column in MSE_Heston_1.keys():
#     rmse_values = [np.sqrt(mse) for mse in MSE_Heston_1[column]]
#     print(f"{column}: {np.nanmean(rmse_values):.3f} ({np.nanstd(rmse_values):.3f})")
# pd.DataFrame(pd.DataFrame(MSE_Heston_1)).to_csv("data/results/MSE_Heston_1_2023_copy.csv", index=False)

MSE_Heston_7, params_Heston_7 = calibrate_model(CALL_7dte, PUT_7dte, T=5/252, model="Heston")
for column in MSE_Heston_7.keys():
    rmse_values = [np.sqrt(mse) for mse in MSE_Heston_7[column]]
    print(f"{column}: {np.nanmean(rmse_values):.3f} ({np.nanstd(rmse_values):.3f})")
pd.DataFrame(pd.DataFrame(MSE_Heston_7)).to_csv("data/results/MSE_Heston_7.csv", index=False)

MSE_Heston_30, params_Heston_30 = calibrate_model(CALL_30dte, PUT_30dte, T=22/252, model="Heston")
for column in MSE_Heston_30.keys():
    rmse_values = [np.sqrt(mse) for mse in MSE_Heston_30[column]]
    print(f"{column}: {np.nanmean(rmse_values):.3f} ({np.nanstd(rmse_values):.3f})")
